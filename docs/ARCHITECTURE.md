...
| **MCP Content-Length stdio** | Cursor's local MCP transport is JSON-RPC with `Content-Length` framing, not ad-hoc NDJSON. `tools/docs_mcp_server.py` speaks that framing (NDJSON still accepted as a fallback). Cloud stdio cannot set `cwd` or expand `${workspaceFolder}` — `.cursor/mcp-diffusers-docs.py` locates the server from cwd / git root / `/workspace`. |
