...
command copies from. For schedulers, the command writes to
`src/diffusers/schedulers/scheduling_<name>.py` (a stand-in for the real
library path) plus `tests/schedulers/test_scheduling_<name>.py`.

The whole point of the kit's design is that **adding a new
