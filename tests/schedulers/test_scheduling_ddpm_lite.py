"""Contract test for the scaffolded scheduler.

Runs on a fresh clone with ZERO third-party installs for the static layers:
  python -m unittest discover -s tests -v

Three layers, deliberately distinct so they don't just re-run the gate:
  1. Structural   (AST, always runs) — a thin sanity check; the gate owns this.
  2. Signature    (AST, always runs) — deeper than the gate: verifies the step()
     and set_timesteps() SIGNATURES (param names/defaults), not just presence.
  3. Behavioral   (needs torch, skipped otherwise) — the real contract: config
     round-trip, timestep count, output type, and seed determinism. These run in
     CI where torch is installed. Skipped != duplicated: they assert BEHAVIOUR,
     which the static gate cannot.
"""
import ast
...
TARGET = REPO / "examples" / "scaffolded_scheduler" / "scheduling_ddpm_lite.py"
CLASS = "DDPMLiteScheduler"
...
class TestBehavioralContract(unittest.TestCase):
    ...
    def setUp(self):
        ...
        spec = importlib.util.spec_from_file_location("scheduling_ddpm_lite", TARGET)
        self.mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(self.mod)
        self.Scheduler = getattr(self.mod, CLASS)

    def test_config_roundtrip(self):
        """@register_to_config must let the scheduler rebuild from its config."""
    ...
    def test_timesteps_count(self):
        """set_timesteps(n) must produce exactly n inference timesteps."""
        s = self.Scheduler()
        s.set_timesteps(25)
        self.assertEqual(len(s.timesteps), 25)

    def test_output_type(self):
        """step() must return a SchedulerOutput exposing prev_sample."""
        import torch
        s = self.Scheduler()
        s.set_timesteps(10)
