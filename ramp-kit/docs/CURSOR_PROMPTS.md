...
screen). Open the `ramp-kit` folder as the Cursor workspace first.

## Pre-flight (20 seconds, so nothing stalls live)

- **Enable the MCP:** Cursor → Settings → **MCP** → enable `diffusers-docs`
  (already wired in `.cursor/mcp.json`) and allow its tool calls.
...
2. Using the /scaffold workflow in .cursor/commands/scaffold.md, scaffold a NEW
   ...
   - Create tests/schedulers/test_scheduling_euler_lite.py from
     tests/_templates/scheduler_test.py (satisfies TEST001).

3. Run the gate on your new file and fix every BLOCKING finding until it's clean:
   `python tools/convention_check.py src/diffusers/schedulers/scheduling_euler_lite.py`

4. Run the tests: `python -m unittest discover -s tests -t .`
...
the PM Definition of Done, the QA checklist, and CI, then restores. This is the
"what happens when you're gone" answer.
```
...
`python tools/build_projections.py` and show that a new .cursor/rules/10-model.mdc
...
- **Correct scaffold:** the new scheduler passes the gate at 0 findings; the math
  is an honest TODO, not fabricated.
- **Multi-audience:** open `projections/pm|qa|devops/` — same rules, different
  surface. Note the upstream-vs-customer split.
