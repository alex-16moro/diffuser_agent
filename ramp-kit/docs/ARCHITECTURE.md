...
| **`.cursorignore` for boundaries** | Requirement 3 says "don't pull in context from outside approved boundaries." `.cursorignore` is exactly that mechanism — it removes paths from the agent's context window, so a scaffold can't copy vendored/secret/generated code toward prod. |
| **`.cursor/hooks.json` `afterFileEdit`** | Runs the gate on the edited file the moment the agent touches it — the same script CI runs — so feedback is instant and CI surprises are rare. |
...
|-------|-------------------|
| Plan | PM Definition of Done, generated from the same rules |
...
| CI / Release gate | Identical gate in `ci-gate.yml` + MCP self-test + tests; `.cursorignore` keeps changes in the agent's context boundary. I stop here — no customer deploy env or auth model, so I don't fake "deploy". |
