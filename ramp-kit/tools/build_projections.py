...
`make build` runs this. It regenerates:
  ...
  projections/devops/ci-gate.yml         (DevOps, GitHub Actions)
...
def build_cursor_core():
    ...
    lines = [
        ...
        "library's current docs before scaffolding. Do not pull in code or context",
        "from outside this repo's approved boundaries (see `.cursorignore`).",
        "",
...
def build_agents_md():
    ...
    lines.extend([
        ...
        "",
        "## First task",
        f"Scaffold a correct first contribution: {comp_list}. See `.cursor/commands/scaffold.md`.",
        "",
    ])
...
def build_devops():
    ...
    write(ROOT / "projections" / "devops" / "ci-gate.yml", "\n".join(yml))
